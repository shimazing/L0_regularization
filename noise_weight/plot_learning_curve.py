...
def best_n_layer_mlp(nhid_list, n_layer):
    ...
    ax.set_title("[{}] {} for MNIST".format(POLICY, ", ".join(model_name_list)))
    plt.savefig("{}_{}_layer_MLP_MNIST.png".format(POLICY, n_layer))
    plt.close()
        ...
        for noise_layer in noise_layer_list:
            ...
            for rand_seed in range(SEED_MAX):
                if epoch=="last":
                    ...
                    ckpt_name = "wine_{}epoch_{}_{}_{}_{}_{}.pth.tar".format(epoch, model_name, policy, noise_layer, "relu", rand_seed)
                try:
                    ckpt = torch.load(os.path.join(CKPT_DIR, ckpt_name))
                except:
                    print(" *** Error opening {}".format(ckpt_name))
