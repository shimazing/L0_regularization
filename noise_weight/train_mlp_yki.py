...
parser.add_argument("--policy", type=str, required=True, choices=["AlternatingNoisyMLP", "IncomingNoisyMLP"])
...
def main():
    ...
    if args.policy == "AlternatingNoisyMLP":
        model = AlternatingNoisyMLP(input_dim=n_features, n_cls=n_cls, layer_dims=layer_dims,
                                    activation_fn=args.act_fn, noise_layer=args.noise_layer)
    elif args.policy == "IncomingNoisyMLP":
        model = IncomingNoisyMLP(input_dim=n_features, n_cls=n_cls, layer_dims=layer_dims,
                                 activation_fn=args.act_fn, noise_layer=args.noise_layer)
    else:
        raise ValueError
    #
