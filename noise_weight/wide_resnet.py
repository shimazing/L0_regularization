    ...
    def __init__(self, depth, widen_factor, dropout_rate, num_classes,
        ...
        for m in self.layer1.modules():
            if isinstance(m, nn.Conv2d):
                m.weight.requires_grad = grad_conf[0]
                m.bias.requires_grad = grad_conf[0]
        for m in self.layer2.modules():
            if isinstance(m, nn.Conv2d):
                m.weight.requires_grad = grad_conf[1]
                m.bias.requires_grad = grad_conf[1]
        for m in self.layer3.modules():
            if isinstance(m, nn.Conv2d):
                m.weight.requires_grad = grad_conf[2]
                m.bias.requires_grad = grad_conf[2]
