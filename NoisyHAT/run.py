...
parser.add_argument('--parameter',type=str,default='4.0,400',help='(default=%(default)s)')
...
if args.experiment=='mnist2' or args.experiment=='pmnist':
    ...
    elif args.approach=="noisy-hat" or args.approach=="noisy-hat-test":
...
for t,ncla in taskcla:
    ...
    # Train
    if args.approach == "hat" or args.approach == "noisy-hat":
        train_loss_list, train_acc_list, valid_loss_list, valid_acc_list = appr.train(task,xtrain,ytrain,xvalid,yvalid)
    else:
    ...
    ckpt_dir = "ckpt"
    token = args.parameter.split(",")
    lamba = float(token[0])
    smax = float(token[1])
    hyperparams = "_".join(args.parameter.split(","))
    ckpt_name = args.experiment + '_' + str(t) + "_" + args.approach + '_' + str(args.nhid) + "_" + str(
        args.seed) + "_" + hyperparams + '.pth.tar'
    state={"experiment":args.experiment,
    ...
    utils.save_checkpoint(state, ckpt_dir, ckpt_name)
