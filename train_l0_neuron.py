...
parser = argparse.ArgumentParser(description='PyTorch MLP-300-100 Training')
...
def main():
    ...
    ckpt_name = "{}_{}_policy_{}_{:.2f}_noise_{:.3f}_{:.2e}_{:.2e}_{:.2e}_{}_{:.2f}.pth.tar".format(
        args.name, args.policy, args.rand_seed, args.sparsity, args.beta_ema,
        args.lambas[0], args.lambas[1], args.lambas[2], args.local_rep, args.temp
    )
    if args.tensorboard:
    ...
    model = L0MLP(input_dim=784, num_classes=10, layer_dims=(layer1_dim, layer2_dim), N=50000,
    ...
    if args.verbose:
        print('Number of model parameters: {}'.format(sum([p.data.nelement() if p.requires_grad else 0 for p in model.parameters()])))
